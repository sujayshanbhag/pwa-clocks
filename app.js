...
async function init() {
    const fetchPromises = config.map(async (c) => {
        try {
            const res = await fetch(`https://timeapi.io/api/Time/current/zone?timeZone=${c.zone}`);
            const data = await res.json();
            
            if (data.isOffline) {
                document.getElementById('status-heading').innerText = "GLOBAL STATIONS (OFFLINE)";
            ...
            return {
                ...
                // We calculate offset normally, but the apiMs is already adjusted by SW
